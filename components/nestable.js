...
(function (addon) {
    ...
    'use strict';
    var hasTouch = 'ontouchstart' in window, html = $('html'), touchedlists = [], $win = UI.$win, draggingElement;
    /**
    ...
    UI.component('nestable', {
        ...
        list: function (options) {
            var data = [], list = this, depth = 0, options = $.extend({}, list.options, options), step = function (level, depth, parent) {
        ...
        dragStart: function (e) {
            var mouse = this.mouse, target = $(e.target), dragItem = target.closest(this.options.itemNodeName), offset = dragItem.offset();
        ...
        dragStop: function (e) {
            ...
            if (this.tmpDragOnSiblings[0] != el[0].previousSibling || this.tmpDragOnSiblings[1] && this.tmpDragOnSiblings[1] != el[0].nextSibling) {
                this.element.trigger('change.uk.nestable', [
                    el,
                    this.hasNewRoot ? 'added' : 'moved'
                ]);
                if (this.hasNewRoot) {
                    this.dragRootEl.trigger(UI.prefix('change.uk.nestable'), [
                        el,
                        'removed'
                    ]);
                }
        ...
        dragMove: function (e) {
            ...
            var pointElRoot = this.element, tmpRoot = this.pointEl.closest('.' + this.options.listBaseClass), isNewRoot = pointElRoot[0] !== this.pointEl.closest('.' + this.options.listBaseClass)[0], $newRoot = tmpRoot;
            /**
             * move vertical
             */
