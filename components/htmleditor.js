...
(function (addon) {
    ...
    require('uikit');
    module.exports = component || addon(jQuery, UIkit);;
}(function ($, UI) {
    'use strict';
    var editors = [];
    UI.component('htmleditor', {
        ...
        init: function () {
            ...
            var previewContainer = this.iframe ? this.preview.container : $this.preview.parent(), codeContent = this.code.find('.CodeMirror-sizer'), codeScroll = this.code.find('.CodeMirror-scroll').on('scroll', UI.Utils.debounce(function () {
                    if ($this.htmleditor.attr('data-mode') == 'tab')
                        return;
                    // calc position
                    var codeHeight = codeContent.height() - codeScroll.height(), previewHeight = previewContainer[0].scrollHeight - ($this.iframe ? $this.iframe.height() : previewContainer.height()), ratio = previewHeight / codeHeight, previewPostition = codeScroll.scrollTop() * ratio;
                    // apply new scroll
        ...
        replaceInPreview: function (regexp, callback) {
            ...
            function translateOffset(offset) {
                ...
                return {
                    ...
                    ch: result[result.length - 1].length
                };
            }
            return results;
        },
        ...
        fit: function () {
            ...
            if (mode == 'tab') {
                ...
                this.htmleditor.find('.@-htmleditor-button-code, .@-htmleditor-button-preview').removeClass('@-active').filter(this.activetab == 'code' ? '.@-htmleditor-button-code' : '.@-htmleditor-button-preview').addClass('@-active');
            }
            this.editor.refresh();
            this.preview.parent().css('height', this.code.height());
        ...
        replaceSelection: function (replace) {
            ...
            if (!text.length) {
                var cur = this.editor.getCursor(), curLine = this.editor.getLine(cur.line), start = cur.ch, end = start;
        ...
        replaceLine: function (replace) {
            var pos = this.editor.getDoc().getCursor(), text = this.editor.getLine(pos.line), html = replace.replace('$1', text);
        ...
        init: function (editor) {
            ...
            var listfn = function () {
                if (editor.getCursorMode() == 'html') {
                    var cm = editor.editor, pos = cm.getDoc().getCursor(true), posend = cm.getDoc().getCursor(false);
    ...
    UI.plugin('htmleditor', 'markdown', {
        init: function (editor) {
            ...
            editor.on('action.listOl', function () {
                if (editor.getCursorMode() == 'markdown') {
                    var cm = editor.editor, pos = cm.getDoc().getCursor(true), posend = cm.getDoc().getCursor(false), prefix = 1;
                    if (pos.line > 0) {
